...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.success) {
                success(state === "login" ? 'Login successful!' : 'Registration successful!')
                navigate('/app')
            } else {
    ...
    <div className='flex items-center justify-center min-h-screen bg-gray-50'>
      <form onSubmit={handleSubmit} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                <button 
                    type="submit" 
                    disabled={isLoading}
                    className="mt-2 w-full h-11 rounded-full text-white bg-indigo-500 hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed">
